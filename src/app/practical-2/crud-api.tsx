...
export default function CrudApiDemoScreen() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        setPosts(data);
      } catch { Alert.alert('Error', 'Failed to fetch posts'); }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setTitle(''); setEditPost(null); setModalVisible(false);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.codeBox}>
        ...
        <Text style={styles.code}>{`GET  axios.get('/posts')\nPOST axios.post('/posts', data)\nPUT  axios.put('/posts/1', data)\nDEL  axios.delete('/posts/1')`}</Text>
      </View>
      ...
      >
        <AppTextInput value={title} onChangeText={setTitle} placeholder="Post title" />
      </AppModal>
    </View>
  );
}
