...
const QUERY = `query { continents { code name countries { name emoji } } }`;

interface Country { name: string; emoji: string; }
...
export default function GraphQLDemoScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      const res  = await fetch(GQL_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: QUERY }) });
      const json = await res.json();
      setData(json.data.continents); setFetched(true);
  ...
  return (
    <View style={styles.container}>
      ...
      {loading && <ActivityIndicator color={ACCENT} size="large" style={{ marginTop: Spacing.xl }} />}
      ...
      {fetched && !loading && (
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity style={styles.continentCard} onPress={() => setExpanded(isOpen ? null : item.code)} activeOpacity={0.8}>
                <View style={styles.continentRow}>
                  <Text style={styles.continentName}>{item.name}</Text>
                  <Text style={styles.continentCount}>{item.countries.length} countries {isOpen ? '▲' : '▼'}</Text>
                </View>
                {isOpen && (
                  <View style={styles.countriesList}>
                    {item.countries.slice(0, 8).map(c => (
                      <Text key={c.name} style={styles.countryItem}>{c.emoji} {c.name}</Text>
                    ))}
                    {item.countries.length > 8 && <Text style={styles.moreText}>+{item.countries.length - 8} more…</Text>}
                  </View>
                )}
              </TouchableOpacity>
            );
